...
openssl req -x509 -newkey rsa:4096 -nodes -days 365 -keyout ca-key.pem -out ca-cert.pem -subj "/C=PT/ST=EUROPE/L=LISBON/O=DEV/OU=TUTORIAL/CN=*.server.dev/emailAddress=trader@tutorial.com"
...
# adding -nodes to not encrypt the private key
openssl req -newkey rsa:4096 -nodes -keyout server-key.pem -out server-req.pem -subj "/C=PT/ST=EUROPE/L=LISBON/O=DEV/OU=TUTORIAL/CN=*.server.com/emailAddress=server@tutorial.com"
...
openssl req -newkey rsa:4096 -nodes -keyout client-key.pem -out client-req.pem -subj "/C=PT/ST=EUROPE/L=LISBON/O=DEV/OU=CLIENT/CN=*.someclient.com/emailAddress=client@gmail.com"
